#!/usr/bin/env python3
"""
Real-Fault Baseline Models.

Trains four sklearn models on the real-fault dataset produced by
build_real_fault_dataset.py and evaluates generalization across:
  Exp A - held-out fault sites (default train/val/test split)
  Exp B - held-out workload (default: irq_test)
  Exp C - leave-one-workload-out cross-validation

Models:
  A. Random Forest
  B. XGBoost (optional; skipped if not installed)
...
Outputs:
  models/rf_binary.pkl   (and similar for each model x task)
  results/baselines_binary.json
  results/baselines_multiclass.json
  results/generalization_exp_b.json
  results/generalization_exp_c.json
  reports/figures/auroc_by_model.png
  reports/figures/confusion_matrices.png
  reports/figures/feature_importance.png
  reports/figures/tsne_visualization.png
  reports/figures/auroc_by_workload.png
  reports/figures/auroc_by_category.png

Usage:
    python src/train_real_fault_baselines.py --data-dir data
    python src/train_real_fault_baselines.py --data-dir data --task binary
"""
...
def get_split_mask(meta: pd.DataFrame, split: str) -> np.ndarray:
    return (meta['split'] == split).values


def get_workload_mask(meta: pd.DataFrame, workload: str) -> np.ndarray:
    return (meta['workload'] == workload).values


def filter_binary(X: np.ndarray, y: np.ndarray, meta: pd.DataFrame, mask: np.ndarray):
    """Keep only clean (0) and observable (1) samples."""
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray,
                    y_prob: np.ndarray, task: str) -> dict:
    metrics = {
...
def make_models(task: str) -> list[tuple[str, object]]:
    models = [
    ...
    if HAS_XGB:
        models.insert(1, ('xgboost', xgb.XGBClassifier(
...
# ---------------------------------------------------------------------------
# Per-workload AUROC breakdown
# ---------------------------------------------------------------------------

def workload_auroc(model, scaler, X_agg: np.ndarray, y: np.ndarray,
                   meta: pd.DataFrame, task: str) -> dict:
    result = {}
    for wl in WORKLOADS:
        ...
        try:
            if task == 'binary':
                ...
                result[wl] = float(roc_auc_score(yi, prob, multi_class='ovr', average='macro'))
        except Exception:
            result[wl] = float('nan')
    return result


def category_auroc(model, scaler, X_agg: np.ndarray, y: np.ndarray,
                   meta: pd.DataFrame, task: str) -> dict:
    result = {}
    for cat in meta['category'].unique():
        ...
        try:
            if task == 'binary':
                ...
                result[cat] = float(roc_auc_score(yi, prob, multi_class='ovr', average='macro'))
        except Exception:
            result[cat] = float('nan')
    return result
...
# ---------------------------------------------------------------------------
# Experiment A: fault-site split
# ---------------------------------------------------------------------------

def run_exp_a(X_agg: np.ndarray, y: np.ndarray, meta: pd.DataFrame,
              models_dir: Path, task: str) -> dict:
    ...
    if task == 'binary':
        ...
        Xtr, ytr, mtr = X_agg[train_mask], y[train_mask], meta[train_mask]
    ...
    for name, clf in make_models(task):
        ...
        results[name] = {**m, 'auroc_by_workload': wl_au, 'auroc_by_category': cat_au,
...
# ---------------------------------------------------------------------------
# Experiment B: held-out workload
# ---------------------------------------------------------------------------

def run_exp_b(X_agg: np.ndarray, y: np.ndarray, meta: pd.DataFrame,
              holdout: str, task: str) -> dict:
...
# ---------------------------------------------------------------------------
# Experiment C: leave-one-workload-out CV
# ---------------------------------------------------------------------------

def run_exp_c(X_agg: np.ndarray, y: np.ndarray, meta: pd.DataFrame,
              task: str) -> dict:
    print(f"\n  Experiment C - leave-one-workload-out CV ({task})")
    fold_results = {}
    for holdout in WORKLOADS:
        r = run_exp_b(X_agg, y, meta, holdout, task)
        fold_results[holdout] = r

    # Aggregate mean +/- std AUROC per model
    summary = {}
    model_names = list(next(v for v in fold_results.values() if v).keys())
    for name in model_names:
        aurocs = [fold_results[wl][name]['auroc']
                  for wl in WORKLOADS
                  if wl in fold_results and name in fold_results[wl]
        ...
        summary[name] = {
            ...
            'per_fold':   {wl: fold_results[wl].get(name, {}).get('auroc', float('nan'))
                           for wl in WORKLOADS},
        }
        print(f"    [{name}] AUROC={summary[name]['auroc_mean']:.4f} +/- {summary[name]['auroc_std']:.4f}")
...
def generate_figures(results_binary: dict, results_multi: dict,
                     X_agg: np.ndarray, y: np.ndarray,
                     meta: pd.DataFrame, reports_dir: Path):
    try:
        import matplotlib
        matplotlib.use('Agg')
    ...
    if results_binary or results_multi:
        ...
        ax.bar(x - w/2, bin_aurocs,   w, label='Binary (clean vs obs)', color='steelblue')
    ...
    if results_binary:
        ...
        for ax, name in zip(axes, model_names):
            ...
            ax.set_xticks(range(len(cm))); ax.set_xticklabels(tick_labels[:len(cm)])
            ax.set_yticks(range(len(cm))); ax.set_yticklabels(tick_labels[:len(cm)])
    ...
    rf_result = results_binary.get('random_forest', {}) or results_multi.get('random_forest', {})
    if 'feature_importances' in rf_result:
        imp = np.array(rf_result['feature_importances'])
        top_n = 30
        idx   = np.argsort(imp)[-top_n:][::-1]
        names = [AGG_FEATURE_NAMES[i] if i < len(AGG_FEATURE_NAMES) else f'feat_{i}' for i in idx]
    ...
    # 4. t-SNE visualization (test split only)
    ...
    if len(Xte) >= 10:
        ...
        try:
            emb = TSNE(n_components=2, random_state=42, perplexity=min(30, n_tsne-1)).fit_transform(Xte[idx])
            ...
            for lbl in [LABEL_CLEAN, LABEL_OBS, LABEL_SILENT]:
                m = yte[idx] == lbl
                if m.sum() > 0:
                    ax.scatter(emb[m, 0], emb[m, 1], c=colors[lbl], label=label_strs[lbl],
                               alpha=0.6, s=15)
    ...
    # 5. AUROC by workload
    if results_binary:
        wl_data = {n: results_binary[n].get('auroc_by_workload', {}) for n in model_names}
        _plot_auroc_breakdown(wl_data, WORKLOADS, 'AUROC by workload (binary)',
                              str(fig_dir / 'auroc_by_workload.png'), plt)

    # 6. AUROC by fault category
    if results_binary:
        categories = list(next(
            v.get('auroc_by_category', {}).keys()
            for v in results_binary.values()
            if v.get('auroc_by_category')
        ) if any(v.get('auroc_by_category') for v in results_binary.values()) else iter([]))
        if categories:
            cat_data = {n: results_binary[n].get('auroc_by_category', {}) for n in model_names}
            _plot_auroc_breakdown(cat_data, categories, 'AUROC by fault category (binary)',
                                  str(fig_dir / 'auroc_by_category.png'), plt)

    print(f"  Figures saved to {fig_dir}/")


def _plot_auroc_breakdown(data: dict, keys: list, title: str, path: str, plt):
...
def main(args) -> int:
    ...
    print(f"\n{'='*64}")
    print("  ITC2 - Real Fault Baseline Training")
    print(f"{'='*64}")
    ...
    for task in tasks:
        ...
        res_a = run_exp_a(X_agg, y, meta, models_dir, task)
        results_by_task[task] = res_a

        res_b = run_exp_b(X_agg, y, meta, args.holdout_workload, task)
        exp_b_by_task[task] = {'holdout_workload': args.holdout_workload, 'results': res_b}

        res_c = run_exp_c(X_agg, y, meta, task)
        exp_c_by_task[task] = res_c
    ...
    for task in tasks:
        ...
        out = {'timestamp': ts, 'task': task,
               'exp_a': _serialise(results_by_task[task])}
        with open(results_dir / fname, 'w') as f:
            json.dump(out, f, indent=2)
        print(f"\n  Saved {fname}")

    with open(results_dir / 'generalization_exp_b.json', 'w') as f:
        json.dump({'timestamp': ts, **_serialise(exp_b_by_task)}, f, indent=2)
    with open(results_dir / 'generalization_exp_c.json', 'w') as f:
        json.dump({'timestamp': ts, **_serialise(exp_c_by_task)}, f, indent=2)
...
def _parse_args():
    ...
    p.add_argument('--holdout-workload',  default='irq_test',
                   help='Workload to hold out in Experiment B')
    return p.parse_args()


# Import feature names for reporting
